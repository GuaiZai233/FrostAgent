...
- **可插拔分类器架构与纯 LLM 安全网关（Pure LLM Gateway Architecture & Option A Fail-Closed）**：
  - 彻底移除了原先硬编码在 `watchdog.go` 内的静态正则表达式列表（`dangerousPatterns`），并遵照 Maintainer 架构裁定**将本地基于规则/正则的语义内容分类器（`CalibratedClassifier`）完全从生产执行路径中剔除**。语义安全分类职责完全收束至 LLM 安全网关。保留确定性安全底座（归一化、来源溯源、内容哈希、规避检测记账、大小限制、AccessStore 状态与审计日志），但语义违规判定由且仅由 LLM 网关完成。
  ...
  - **未配置/故障严格 Option A Fail-Closed**：当系统未配置 LLM 提供者（`provider == nil` 或 `classifier == nil`）或实例未装配网关时，系统坚决拒绝回退到本地弱规则，而是直接执行 Option A 严格 Fail-Closed 阻断（`WatchdogBlock`，0 strike，不封禁）。同样，当 LLM 出现超时、上游断连、网络报错或返回畸形/非结构化 JSON 时，一律执行严格 Fail-Closed 阻断，杜绝无网关或网关异常时的安全降级放行。
  - **`CalibratedClassifier` 仅作为离线测试桩（Offline Test Stub / Benchmark Fixture）**：`CalibratedClassifier` 严格作为离线单元测试和对抗评测基准套件（`internal/security/eval`）的确定性测试桩，用于在无网络和无真实模型调用的测试环境下严密检验确定性安全底座流水线（归一化、溯源打标、编码规避升级、Lock 阈值阶梯和审计脱敏），完全退出生产链路。
...
3. **Pure LLM Gateway Architecture & Option A Fail-Closed**: Completely eliminated local regex/keyword semantic content classification (`CalibratedClassifier`) from all production paths. Semantic risk evaluation is exclusively delegated to the LLM security gateway (`LLMClassifier` via `core.LLMProvider` with XML entity escaping across `&`, `<`, `>`, `"`, `'` to guarantee delimiter breakout immunity). When no LLM provider is configured (or provider is nil), the system strictly fails closed under Option A (`BLOCK`, zero strikes, no lock). Deterministic plumbing (normalization, provenance tracking, content hashing, evasion bookkeeping, limits, access store, audit) is preserved. `CalibratedClassifier` is retained solely as an offline test fixture and benchmark stub for verifying deterministic policy plumbing in unit tests. In multi-instance runtimes, security classifier providers are scoped per-instance (`SetInstanceProvider`/`RemoveInstanceProvider`) as direct `*LLMClassifier` instances and synchronized via `sync.RWMutex`, eliminating interface data races under concurrent reloads while preserving unified global `AccessStore` and `AuditStore` semantics.
...
8. **Offline Benchmark & Policy Test Fixture**: `CalibratedClassifier` is retained strictly as an offline benchmark fixture and test stub for unit tests verifying deterministic policy plumbing (strikes, locks, tail smuggling, percent escapes, zero-width encodings) without requiring live network or model dependencies. Platform policy patterns in the stub disambiguate isolated tokens (e.g. removing standalone `\bcp\b`), ensuring tests accurately validate policy flows.
