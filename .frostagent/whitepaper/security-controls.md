...
- **对抗评估基准体系（Adversarial Evaluation Framework）**：
  ...
  - **解耦确定性策略底座测试与语义网关评估**：
    ...
    - 针对语义安全分类器评估：
      - **流水线契约回归（`TestAdversarialCorpusPipelineContract`）**：在离线 CI 环境中使用确定性 Mock Provider 检验生产级 `*security.LLMClassifier` 完整流水线契约（XML 实体转义、Prompt 组装、JSON 反序列化、结构校验与 Watchdog 决策闭环）。此测试定位为工程管线回归测试，明确不声明为生产大模型的真实语义辨识质量指标。
      - **生产模型实测通道（`TestLiveGatewayAdversarialEvaluation`）**：提供独立的可选（opt-in）真机评测路径，当提供专用环境变量 `FROSTAGENT_SECURITY_EVAL_API_KEY`（或在显式开启 `FROSTAGENT_SECURITY_EVAL_LIVE=1` 时复用 `OPENAI_API_KEY`）时接入真实配置的 LLM 安全模型进行全量基准测试，实测并输出特定模型的误阻断率（False Block Rate）、误封禁率（Zero-Tolerance False Lock Rate）与严重漏检率（Severe Miss Rate）；未显式 opt-in 或无密钥时自动跳过（`t.Skip`），杜绝常规开发机因环境残留 `OPENAI_API_KEY` 而在无意中执行付费网络评测，不阻塞普通 PR CI。在执行具体生产模型评测前，语义模型质量指标保持未声明状态。
