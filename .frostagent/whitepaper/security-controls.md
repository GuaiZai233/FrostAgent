...
- **对抗评估基准体系（Adversarial Evaluation Framework）**：
  ...
  - **解耦确定性策略底座测试与语义网关评估**：
    ...
    - 针对语义安全分类器评估：
      ...
      - **生产模型实测通道（`TestLiveGatewayAdversarialEvaluation`）**：提供独立的可选（opt-in）真机评测路径，当提供环境变量 `FROSTAGENT_SECURITY_EVAL_API_KEY`（或 `OPENAI_API_KEY`）时接入真实配置的 LLM 安全模型进行全量基准测试，实测并输出特定模型的误阻断率（False Block Rate）、误封禁率（Zero-Tolerance False Lock Rate）与严重漏检率（Severe Miss Rate）；无密钥时自动跳过（`t.Skip`），不阻塞普通 PR CI。在执行具体生产模型评测前，语义模型质量指标保持未声明状态。
