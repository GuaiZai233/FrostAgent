...
- **可插拔分类器架构与 LLM 安全网关（Classifier Gateway & Hybrid Fallback）**：
  ...
  - 抽象出标准 `Classifier` 接口，支持多层级安全审查组件：
    - **`LLMClassifier`（语义安全网关）**：基于 `core.LLMProvider` 驱动的高鲁棒性安全网关，采用 `<content>...</content>` 隔离定界符与 `EscapeXML` 实体转义（对 `&`、`<`、`>`、`"`、`'` 进行全量字符实体编码）防御定界符注入逃逸，彻底杜绝攻击者闭合 `</content>` 并注入伪造系统指令的攻击面；通过严格结构化 JSON 输出对输入文本进行语义级越狱、注入意图、破坏性命令与数据窃取分析，天然具备极高跨语言防御能力。
    - **`CalibratedClassifier`（多语言校准分类器）**：本地确定性多语言分类器，涵盖英、中、意、俄、日、西、法、德等主流语言提示词注入与越狱规则、破坏性执行指令（反弹 Shell、破坏性删除、Fork Bomb）、凭据收割以及腾讯平台合规策略规则，内置**教育与分析上下文检测**（识别 "what is rm -rf"、"如何防范提示词注入" 等良性科普讨论），彻底杜绝敏感词盲目误报。该分类器严格保留用于离线环境或未配置 LLM 的模式。
    - **`HybridClassifier`（生产级严格 Fail-Closed 混合分类器 - Option A）**：在生产已配置 LLM 安全网关的场景下，严格执行 Fail-Closed 策略。当 LLM 出现超时、上游断连、网络报错或返回畸形/非结构化 JSON 时，不再静默吞咽错误或降级至本地轻量规则，而是直接向 `Watchdog` 向上抛出错误；本地确定性 `CalibratedClassifier` 严格仅用于未配置 LLM（`h.llm == nil`）的离线模式，从根源杜绝模型故障引发的潜在降级放行。
    ...
    - **严格结构化输出与必填字段强校验（Strict Required-Field Structural Validation）**：LLM 返回的 JSON 载荷解析采用指针字段（`*string`、`*float64`）严格区分显式赋值与缺省零值，在枚举规范化前强制验证必填字段（`category`、`risk_level`、`intent`、`confidence`）非空且必须存在。彻底移除了空字符串向有效枚举（如 `none`、`benign`）的隐式映射；空对象 `{}`、部分缺失字段（如缺少 `intent`、`confidence`）一律被拒绝并视作分类器失败，由 `HybridClassifier` 严格向上报告错误，杜绝恶意截断伪装放行。
    ...
    - **入口单次分类调用优化（Ingress Single-Call Path Optimization）**：在 `Watchdog.EvaluateWithContext` 评估直接输入时，预先比对原始输入 `rawContent` 与归一化输入 `normalized`。在绝大多数良性输入未触发转义修改（`rawContent == normalized`）的常见场景下，仅发起一次分类器调用并复用结果，将模型网关的耗时与成本降低 50%；唯有当归一化实际发生了字符转换（`rawContent != normalized`）时，才触发第二次针对原始文本的分类审查，以精准识别编码规避行为。
...
3. **Pluggable Classifier Gateway & Multi-Instance Scoping**: Replaced fragile static regex lists with a pluggable `Classifier` architecture featuring `LLMClassifier` (semantic security gateway via `core.LLMProvider` with `EscapeXML` entity escaping across `&`, `<`, `>`, `"`, `'` to guarantee delimiter breakout immunity), `CalibratedClassifier` (deterministic multilingual classifier with educational intent detection, reserved for offline/no-LLM mode), and `HybridClassifier` (strictly fail-closed under Option A: propagating upstream errors/timeouts/malformed JSON directly to `Watchdog` without swallowing or silent degradation). In multi-instance runtimes, security classifier providers are scoped per-instance (`SetInstanceProvider`/`RemoveInstanceProvider`) and synchronized via `sync.RWMutex`, preventing last-writer mutation across instances and eliminating Go interface data races under concurrent reloads while preserving unified global `AccessStore` and `AuditStore` semantics.
...
8. **Zero-False-Lock Fallback Invariant**: Platform policy patterns in `CalibratedClassifier` disambiguate isolated tokens (e.g. removing standalone `\bcp\b`), guaranteeing that ordinary system commands (such as Linux `cp a.txt b.txt`) pass through the deterministic fallback without false blocks or locks.
...
12. **Adversarial Evaluation Framework**: Native evaluation runner (`internal/security/eval`) measuring False Block Rate, False Lock Rate (Zero Tolerance), and Severe Miss Rate across multilingual benchmark corpuses.
